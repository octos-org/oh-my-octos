...
## 1. `on_turn_end` hook event is documented but never fired — filed as https://github.com/octos-org/octos/issues/2244
...
## 2. Skill-manifest hooks: only `command[0]` is resolved against the skill directory — filed as https://github.com/octos-org/octos/issues/2245
...
## 3. `session_id` / `profile_id` missing from hook payloads on `octos chat` and `octos serve` — filed as https://github.com/octos-org/octos/issues/2246
...
## 4. `octos skills list` prints its table to stderr — filed as https://github.com/octos-org/octos/issues/2247

`octos skills list 2>/dev/null` prints nothing; `skills info` uses stdout. Scripts that pipe `skills list` get an empty result.

## 5. Unix socket under the data dir limits `--data-dir` path length — filed as https://github.com/octos-org/octos/issues/2248
...
## 6. A `before_llm_call` deny is logged as an internal harness error — filed as https://github.com/octos-org/octos/issues/2249
...
## 7. `octos skills install user/repo/skill` only looks at the repo root — filed as https://github.com/octos-org/octos/issues/2250
