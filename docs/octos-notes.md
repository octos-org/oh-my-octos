...
- Only `command[0]` is resolved against the skill directory (when it starts with `./`). Any later argv element is left as-is and resolves against the workspace (upstream #2245). Hence `["./main", "hook", "<name>"]`: the binary is argv[0], the hook name is a plain argument.
...
- Skill binaries: `octos skills install` runs `maybe_install_binary` for every skill with a `manifest.json` or `Cargo.toml`, tools or not. Order: `manifest.binaries["<os>-<arch>"]` (os is `darwin`/`linux`/`windows`, arch `aarch64`/`x86_64`; a `.tar.gz` has its first file extracted, a bare URL is saved as-is) → registry → `cargo build --release` with `target/release/<package name>` copied to `main`. An existing `main` skips all three, which is how the e2e suite stages a freshly built binary.
