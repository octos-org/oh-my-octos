...
class EditCheck(unittest.TestCase):
    ...
    def test_invalid_json(self):
        rel = self.write("cfg.json", '{"a": [1, 2}')
        rc, out, _ = run_hook("edit_check.py", self.payload(rel, tool="edit_file"), cwd=self.dir)
        self.assertEqual(rc, 1)
        self.assertIn("invalid JSON", out)

    def test_json_with_comments_and_trailing_commas_is_tolerated(self):
        rel = self.write("tsconfig.json", '{\n  // comment\n  "compilerOptions": {"strict": true,},\n  /* block */\n}')
        rc, out, _ = run_hook("edit_check.py", self.payload(rel), cwd=self.dir)
        self.assertEqual((rc, out), (0, ""))
        rel = self.write(".vscode/settings.json", '{"a": 1, // trailing\n}')
        rc, out, _ = run_hook("edit_check.py", self.payload(rel), cwd=self.dir)
        self.assertEqual((rc, out), (0, ""))

    def test_jsonc_that_is_really_broken_still_reports(self):
        rel = self.write("tsconfig.json", '{ // c\n "a": [1, }')
        rc, out, _ = run_hook("edit_check.py", self.payload(rel), cwd=self.dir)
        self.assertEqual(rc, 1)
        self.assertIn("tolerated", out)

    def test_binary_file_is_skipped(self):
        p = os.path.join(self.dir, "img.json")
        with open(p, "wb") as f:
            f.write(b"\x89PNG\r\n\x1a\n\x00\x00garbage{")
        rc, out, _ = run_hook("edit_check.py", self.payload("img.json"), cwd=self.dir)
        self.assertEqual((rc, out), (0, ""))

    def test_zsh_shebang_uses_zsh_or_skips(self):
        rel = self.write("z.sh", "#!/usr/bin/env zsh\nif [[ 1 ]]; then\n  echo x\nfi\n")
        rc, out, _ = run_hook("edit_check.py", self.payload(rel), cwd=self.dir)
        self.assertEqual((rc, out), (0, ""))

    def test_fish_shebang_is_skipped(self):
        rel = self.write("f.sh", "#!/usr/bin/env fish\nif test 1\n  echo x\nend\n")
        rc, out, _ = run_hook("edit_check.py", self.payload(rel), cwd=self.dir)
        self.assertEqual((rc, out), (0, ""))

    def test_non_ascii_path(self):
        rel = self.write("目录/坏.py", "def f(:\n")
        rc, out, _ = run_hook("edit_check.py", self.payload(rel), cwd=self.dir, env={"LC_ALL": "C", "LANG": "C"})
        self.assertEqual(rc, 1)
        self.assertIn("python syntax error", out)
...
class CostGuard(unittest.TestCase):
    ...
    def test_new_session_in_same_process_resets_bucket(self):
        # No session_id: the bucket is the parent pid. A lower cumulative cost = a new session.
        run_hook("cost_guard.py", {"event": "after_llm_call", "session_cost": 0.06}, env=self.env)
        rc, _, _ = run_hook("cost_guard.py", {"event": "before_llm_call"}, env=self.env)
        self.assertEqual(rc, 1, "first session over budget")
        run_hook("cost_guard.py", {"event": "after_llm_call", "session_cost": 0.001}, env=self.env)
        rc, _, _ = run_hook("cost_guard.py", {"event": "before_llm_call"}, env=self.env)
        self.assertEqual(rc, 0, "a new session starting at a lower cumulative must be allowed")

    def test_stale_state_is_ignored(self):
        import glob, json as _json, time as _time
        run_hook("cost_guard.py", {"event": "after_llm_call", "session_id": self.sid, "session_cost": 9}, env=self.env)
        files = glob.glob(os.path.join(self.tmp, "oh-my-octos", "*.json"))
        self.assertEqual(len(files), 1)
        d = _json.load(open(files[0])); d["updated"] = _time.time() - 5 * 3600
        _json.dump(d, open(files[0], "w"))
        rc, _, _ = run_hook("cost_guard.py", {"event": "before_llm_call", "session_id": self.sid}, env=self.env)
        self.assertEqual(rc, 0, "spend older than the stale window must not deny")
...
class ProjectContext(unittest.TestCase):
    ...
    def test_slow_git_stays_inside_the_hook_timeout(self):
        # A fake `git` that hangs: the hook must give up within its budget and exit 0.
        import time
        fake = os.path.join(self.dir, "bin"); os.makedirs(fake)
    ...
    def test_non_ascii_agents_md_under_c_locale(self):
        with open(os.path.join(self.dir, "AGENTS.md"), "w", encoding="utf-8") as f:
            f.write("项目暗号：白鹭-9\n")
        rc, out, _ = run_hook("project_context.py", self.payload(), cwd=self.dir, env={"LC_ALL": "C", "LANG": "C"})
        self.assertEqual(rc, 0)
        self.assertIn("白鹭-9", out)
